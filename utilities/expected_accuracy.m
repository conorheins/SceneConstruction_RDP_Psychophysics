...
%                       1) both RDP motions are seen certainly (so the scene is
%                       100% guessable)
...
p = exp(-16); % to prevent numerical overflow in the log

eAcc = sum(exp(log(perceptual_probs + p) + log(accuracies + p)));
