function [Scr,inf, myVar] = SetUpConstants_RDP(Scr,inf)
...
Scr.mainDir = pwd;
Scr.expDir = fullfile(Scr.mainDir,'MotionDiscrim');
% Scr.rootDir = pwd;
...
myVar.nDots    = ceil(20/(Scr.pixelsperdegree.^2) .* prod(myVar.apSize) ./ (1/myVar.lifetime_ms) ) ;  % number of dots (density of dots in dots / squared-degree x 1/(ppd^2) x Area of aperture in pixels x lifetime in seconds) 
...
myVar.UPrect = [2*Scr.wRect(3)/4 - 200 3*Scr.wRect(4)/4 2*Scr.wRect(3)/4 - 100 3*Scr.wRect(4)/4 + 85];
myVar.DOWNrect = [3*Scr.wRect(3)/4 - 200 3*Scr.wRect(4)/4 3*Scr.wRect(3)/4 - 100  3*Scr.wRect(4)/4 + 85];
myVar.RIGHTrect  = [Scr.wRect(3) - 200 3*Scr.wRect(4)/4 Scr.wRect(3) - 100  3*Scr.wRect(4)/4 + 85];
